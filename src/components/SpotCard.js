...
function getSpotImageUrl(spot) {
  ...
  return 'https://via.placeholder.com/288x192.png?text=No+Image';
}

export function SpotCard({ spot }) {
  const [isFavorited, setIsFavorited] = useState(false);
  const [isToggling, setIsToggling] = useState(false);
  // spots 테이블의 카운트 컬럼 사용 (초기값)
  const [recommendationCount, setRecommendationCount] = useState(spot.recommendation_count || 0);
  ...
  useEffect(() => {
    ...
    if (guestUserId && spot.id) {
      // 찜 상태 확인
      const apiBase = typeof window !== 'undefined' ? (process.env.NEXT_PUBLIC_API_URL || '/api') : '/api';
      fetch(`${apiBase}/place-favorite/check?guest_user_id=${guestUserId}&place_id=${spot.id}&place_type=spot`)
        .then(async (res) => {
  ...
  async function handleRecommendation(e) {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        // 성공 시 카운트 증가 (spots 테이블이 자동 업데이트되므로 +1)
